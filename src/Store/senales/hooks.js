...
export const useSetLuz = () => {
  const dispatch = useDispatch();
  return (idSenal, luz, estado) => dispatch(setLuzEstado(idSenal, luz, estado));
};

export const useSenalManual = (idCelda) => {
  const senalIsManual = useSelector((state) =>
    selSenalIsManual(state, idCelda)
  );
  const dispatch = useDispatch();
